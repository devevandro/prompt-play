...
export function Visualizer({
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between bg-muted/30 px-4 py-2 font-mono text-terminal-gray text-xs">
        <div className="flex items-center gap-4">
          {isPlaying ? (
            <span className="text-terminal-green">● LIVE</span>
          ) : (
            <span className="text-terminal-yellow">○ IDLE</span>
          )}
          <span>FFT: 48 bands</span>
          {isAudioConnected && (
            <span className="text-terminal-green">● Web Audio API</span>
          )}
        </div>
        <div className="flex items-center gap-4">
          <span>SR: 44.1kHz</span>
          <span>16bit</span>
          {!isAudioConnected && isPlaying && (
            <span className="text-terminal-yellow">~ Procedural</span>
          )}
        </div>
      </div>
    </div>
